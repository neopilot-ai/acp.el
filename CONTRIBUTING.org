...
When submitting a PR, please keep the checkboxes from the [[https://github.com/neopilot-ai/acp.el/blob/main/.github/PULL_REQUEST_TEMPLATE.md][PR template]] and check them off as applicable.
...
As the maintainer, I must be mindful of all features I accept since I inherit the code to maintain it. Some features may be better suited as separate packages (like [[https://github.com/cmacrae/acp-sidebar][acp-sidebar]]).
...
=acp.el= is getting large. When adding substantial new functionality, move helper functions to a dedicated file (e.g. =acp-completion.el=). You can keep the same =acp--= function name prefix. See [[https://github.com/neopilot-ai/acp.el/commit/c60203934d577e88d03035038a1e247bb1ce0ded][this commit]] for an example of this transition.
...
** New state fields go through =acp--make-state=

When adding state, declare it in =acp--make-state= so we get an overall picture of all values available in state.
...
#+RESULTS:
| tests/acp-anthropic-tests.el |
| tests/acp-openai-tests.el    |
| tests/acp-tests.el           |

** Running tests

Opening any file under the =tests= directory will load the =acp-run-all-tests= command.

Run tests with =M-x acp-run-all-tests=.
