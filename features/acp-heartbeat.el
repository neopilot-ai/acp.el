;;; acp-heartbeat.el --- Heartbeat utilities -*- lexical-binding: t; -*-
...
;;; Commentary:
;;
;; Provides heartbeat functionality for acp.
...
(cl-defun acp-heartbeat-make (&key start (beats-per-second 10) on-heartbeat)
...
Returns an alist of the form:
  ...
  (let* ((heartbeat-state (list (cons :heartbeat-timer nil)
                                ...
                                (cons :status nil))))
    (when start
      (acp-heartbeat-start :heartbeat heartbeat-state))
    heartbeat-state))

(cl-defun acp-heartbeat-start (&key heartbeat)
...
(cl-defun acp-heartbeat-stop (&key heartbeat)
...
(provide 'acp-heartbeat)

;;; acp-heartbeat.el ends here
