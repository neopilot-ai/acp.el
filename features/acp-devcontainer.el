;;; acp-devcontainer.el --- Devcontainer support -*- lexical-binding: t; -*-
...
(declare-function acp-cwd "acp")

(defun acp-devcontainer--get-workspace-path (cwd)
...
(defun acp-devcontainer-resolve-path (path)
  "Resolve PATH from a devcontainer in the local filesystem, and vice versa.

For example:

- /workspace/README.md => /home/neopilot-ai/projects/kitchen-sink/README.md
- /home/neopilot-ai/projects/kitchen-sink/README.md => /workspace/README.md"
  (let* ((cwd (acp-cwd))
         (devcontainer-path (acp-devcontainer--get-workspace-path cwd)))
    (if (string-prefix-p cwd path)
        (string-replace cwd devcontainer-path path)
      (if acp-text-file-capabilities
...
(defalias
  'acp--resolve-devcontainer-path
  #'acp-devcontainer-resolve-path)

(provide 'acp-devcontainer)

;;; acp-devcontainer.el ends here
