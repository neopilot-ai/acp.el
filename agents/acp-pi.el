;;; acp-pi.el --- Pi coding agent configurations -*- lexical-binding: t; -*-
...
(defun acp-pi-make-agent-config ()
...
Returns an agent configuration alist using `acp-make-agent-config'."
  (acp-make-agent-config
   ...
   :welcome-function #'acp-pi--welcome-message
   :client-maker (lambda (buffer)
                   (acp-pi-make-client :buffer buffer))
...
(defun acp-pi-start-agent ()
  "Start an interactive Pi coding agent shell."
  (interactive)
  (acp--dwim :config (acp-pi-make-agent-config)
                     :new-shell t))

(cl-defun acp-pi-make-client (&key buffer)
...
environment variables are required by default."
  ...
  (when (and (boundp 'acp-pi-command) acp-pi-command)
    (user-error "Please migrate to use acp-pi-acp-command and eval (setq acp-pi-command nil)"))
  (acp--make-acp-client :command (car acp-pi-acp-command)
                                :command-params (cdr acp-pi-acp-command)
                                :environment-variables acp-pi-environment
                                :context-buffer buffer))

(defun acp-pi--welcome-message (config)
  "Return Pi welcome message using `shell-maker' CONFIG."
  (let ((art (acp--indent-string 4 (acp-pi--ascii-art)))
