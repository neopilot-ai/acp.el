;;; acp-ui.el --- Interactive shell UI elements -*- lexical-binding: t; -*-
...
(defvar-local acp-ui--content-store nil
...
(cl-defun acp-ui-make-fragment-model (&key (namespace-id "global") (block-id "1") label-left label-right body)
...
NAMESPACE-ID, BLOCK-ID, LABEL-LEFT, LABEL-RIGHT, and BODY are the keys."
  (list (cons :namespace-id namespace-id)
        ...
        (cons :label-left (acp-ui--string-or-nil label-left))
        (cons :label-right (acp-ui--string-or-nil label-right))
        (cons :body (acp-ui--string-or-nil body))))

(defun acp-ui--insert-read-only (text)
...
(cl-defun acp-ui-update-fragment (model &key append create-new on-post-process navigation expanded no-undo)
    ...
    (let* ((inhibit-read-only t)
      ...
      (when (or new-label-left new-label-right new-body)
        ...
        (if (and match (not create-new))
            ...
            (let* ((existing-model (acp-ui--read-fragment-at-point))
                   (state (get-text-property (point) 'acp-ui-state))
              ...
              (goto-char block-start)
              (acp-ui--insert-fragment final-model qualified-id
          ...
          (goto-char (point-max))
          (setq padding-start (point))
          (acp-ui--insert-read-only (acp-ui--required-newlines 2))
          (setq block-start (point))
          (acp-ui--insert-fragment model qualified-id expanded navigation)
          (acp-ui--insert-read-only "\n\n")
          (setq padding-end (point))))
      (when on-post-process
        (funcall on-post-process))
      (when-let ((block-range (acp-ui--block-range :position block-start)))
        (list (cons :block block-range)
              (cons :body (acp-ui--nearest-range-matching-property
                           :property 'acp-ui-section :value 'body
                           :from (map-elt block-range :start)
                           :to (map-elt block-range :end)))
              (cons :label-left (acp-ui--nearest-range-matching-property
                                 :property 'acp-ui-section :value 'label-left
                                 :from (map-elt block-range :start)
                                 :to (map-elt block-range :end)))
              (cons :label-right (acp-ui--nearest-range-matching-property
                                  :property 'acp-ui-section :value 'label-right
                                  :from (map-elt block-range :start)
                                  :to (map-elt block-range :end)))
...
(defun acp-ui--read-fragment-at (position qualified-id)
  "Read fragment at POSITION with QUALIFIED-ID."
  (when-let ((fragment (list (cons :block-id qualified-id)))
             (state (get-text-property position 'acp-ui-state))
             (range (acp-ui--block-range :position position)))
    ...
    (save-mark-and-excursion
      (save-restriction
        ...
        (when-let ((label-left (acp-ui--nearest-range-matching-property
                                :property 'acp-ui-section :value 'label-left)))
          (setf (map-elt fragment :label-left) (buffer-substring (map-elt label-left :start)
                                                                 (map-elt label-left :end))))
        (when-let ((label-right (acp-ui--nearest-range-matching-property
                                 :property 'acp-ui-section :value 'label-right)))
          (setf (map-elt fragment :label-right) (buffer-substring (map-elt label-right :start)
                                                                  (map-elt label-right :end))))
        (when acp-ui--content-store
          (when-let ((body (gethash (concat qualified-id "-body") acp-ui--content-store)))
            (setf (map-elt fragment :body) body)))))
    fragment))

(cl-defun acp-ui-delete-fragment (&key namespace-id block-id no-undo)
    ...
    (let* ((inhibit-read-only t)
      ...
      (when match
        (let ((block-start (prop-match-beginning match))
              (block-end (prop-match-end match)))
          (when acp-ui--content-store
            (remhash qualified-id acp-ui--content-store))
...
(defun acp-ui--read-fragment-at-point ()
  "Read fragment at point, returning model or nil if none found."
  (when-let ((state (get-text-property (point) 'acp-ui-state))
             (range (acp-ui--block-range :position (point))))
    (acp-ui--read-fragment-at (map-elt range :start)
                                      (map-elt state :qualified-id))))

(cl-defun acp-ui--block-range (&key position)
...
In the form:
  ...
  ((start . 1)
   (end . 3))."
  (when-let ((qualified-id (map-elt (get-text-property (or position (point)) 'acp-ui-state) :qualified-id)))
    (acp-ui--nearest-range-matching-property
     :property 'acp-ui-state
     :value qualified-id
     :predicate (lambda (qualified-id property)
                  (equal (map-elt property :qualified-id) qualified-id)))))

(cl-defun acp-ui--nearest-range-matching-property (&key property value (predicate t) from to)
...
(defun acp-ui--insert-fragment (model qualified-id &optional expanded navigation)
  ...
  (let ((block-start (point))
    ...
    (when-let ((has-labels (or label-left label-right)))
      (if body
          (progn
            ...
            (insert (acp-ui-add-action-to-text
                     (if expanded "▼ " "▶ ")
                     (lambda ()
                       (interactive)
                       (acp-ui-toggle-fragment-at-point))
                     (lambda ()
                       (message "Press RET to toggle"))))
            (setq indicator-end (point))
            (add-text-properties indicator-start indicator-end
                                 `(acp-ui-section indicator
                                                          keymap ,(acp-ui-make-action-keymap
                                                                   (lambda ()
                                                                     (interactive)
                                                                     (acp-ui-toggle-fragment-at-point)))
                                                          read-only t
        ...
        ;; For example:
        ;;
        ;; "   [ completed ] [ read ] Read acp/README.org"
        ;;
        ;; vs
        ;;
        ;; "▼  [ completed ] [ read ] Read acp/README.org"
    ...
    (when body
      ...
      (let ((clean-body (string-remove-prefix "  " body)))
        (insert (acp-ui--indent-text clean-body "  ")))
      (setq body-end (point))
        ...
        (when (re-search-backward "[^ \t\n]" body-start t)
          ...
          (when (< (point) body-end)
            (add-text-properties (point) body-end
                                 '(invisible t))))))
    (when body
      (unless acp-ui--content-store
        (setq acp-ui--content-store (make-hash-table :test 'equal)))
      (puthash (concat qualified-id "-body") body acp-ui--content-store))
    (put-text-property
     block-start (or body-end label-right-end label-left-end)
     'acp-ui-state (list
                            ;; Note: Avoid storing chunky data in
                            ;; acp-ui-state as it will impact performance.
                            ;; Use acp-ui--content-store for these instances.
...
(cl-defun acp-ui-update-text (&key namespace-id block-id text append create-new no-undo)
...
When NO-UNDO is non-nil, disable undo recording."
  (save-mark-and-excursion
    (let* ((inhibit-read-only t)
           (buffer-undo-list (if no-undo t buffer-undo-list))
           (qualified-id (format "%s-%s" namespace-id block-id))
           (props `(acp-ui-state ((:qualified-id . ,qualified-id))
                                         read-only t
                                         front-sticky (read-only)))
           (match (save-mark-and-excursion
                    (goto-char (point-max))
...
(defun acp-ui--required-newlines (desired)
...
(defun acp-ui-toggle-fragment-at-point ()
  ...
  (save-mark-and-excursion
    (when-let* ((inhibit-read-only t)
                (buffer-undo-list t)
                (state (get-text-property (point) 'acp-ui-state))
                (block (acp-ui--block-range :position (point)))
                (body (acp-ui--nearest-range-matching-property
                       :property 'acp-ui-section :value 'body
                       :from (map-elt block :start)
                       :to (map-elt block :end)))
                (indicator (acp-ui--nearest-range-matching-property
                            :property 'acp-ui-section :value 'indicator
                            :from (map-elt block :start)
                            :to (map-elt block :end)))
                ;; Find where labels end (either label-right or label-left)
                (invisible-start (or (map-elt (acp-ui--nearest-range-matching-property
                                               :property 'acp-ui-section :value 'label-right
                                               :from (map-elt block :start)
                                               :to (map-elt block :end))
                                              :end)
                                     (map-elt (acp-ui--nearest-range-matching-property
                                               :property 'acp-ui-section :value 'label-left
                                               :from (map-elt block :start)
                                               :to (map-elt block :end))
                                              :end)))
      ...
      (let ((new-collapsed-state (not (map-elt state :collapsed))))
        ...
        (put-text-property (map-elt block :start)
                           (map-elt block :end) 'acp-ui-state state)))))

(defun acp-ui-collapse-fragment-by-id (namespace-id block-id)
  ...
  (save-mark-and-excursion
    (let ((qualified-id (format "%s-%s" namespace-id block-id)))
      ...
      (when (text-property-search-backward
             'acp-ui-state qualified-id
             (lambda (_ state)
               (equal (map-elt state :qualified-id) qualified-id))
             t)
        (acp-ui-toggle-fragment-at-point)))))

(defun acp-ui--string-or-nil (str)
  "Return STR if it is not nil and not empty, otherwise nil."
  (and str (not (string-empty-p str)) str))

(defun acp-ui--indent-text (text &optional indent-string)
...
(defun acp-ui-make-action-keymap (action)
...
(defun acp-ui-add-action-to-text (text action &optional on-entered face)
...
FACE when non-nil applies the specified face to the text."
  (add-text-properties 0 (length text)
                       `(keymap ,(acp-ui-make-action-keymap action))
                       text)
  (when on-entered
    (add-text-properties 0 (length text)
...
(defvar-local acp-ui--isearch-opened-fragments nil
  "List of fragment qualified-ids that were opened during isearch.")

(defun acp-ui--isearch-filter-predicate (beg end)
    ...
    (when found-invisible
      (save-excursion
        ...
        (when-let* ((state (get-text-property (point) 'acp-ui-state))
          ...
          (unless (member qualified-id acp-ui--isearch-opened-fragments)
            (push qualified-id acp-ui--isearch-opened-fragments))
          ;; Expand the fragment
          (acp-ui-toggle-fragment-at-point))))
...
(defun acp-ui--isearch-cleanup ()
  "Clean up isearch state when search ends."
  (setq acp-ui--isearch-opened-fragments nil))

(defvar acp-ui-mode-map
  (let ((map (make-sparse-keymap)))
    map)
  "Keymap for `acp-ui-mode'.")

;;;###autoload
(define-minor-mode acp-ui-mode
  "Minor mode for SUI block navigation."
  :lighter " SUI"
  :keymap acp-ui-mode-map
  (if acp-ui-mode
      (progn
        ...
        (setq-local isearch-filter-predicate #'acp-ui--isearch-filter-predicate)
        ;; Clean up when search ends
        (add-hook 'isearch-mode-end-hook #'acp-ui--isearch-cleanup nil 'local))
    (cursor-sensor-mode -1)
    (kill-local-variable 'search-invisible)
    (kill-local-variable 'isearch-filter-predicate)
    (remove-hook 'isearch-mode-end-hook #'acp-ui--isearch-cleanup 'local)))
